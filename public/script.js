...
function renderizarTotais(campanhas) {
    ...
    if (Object.keys(totaisPorMoeda).length === 0) {
        metricsHTML = '<p style="text-align:center;">Nenhum dado para os totais com os filtros atuais.</p>';
    }
    ...
    if (campanhas.length === 0) {
        ...
        campanhas.forEach(campanha => {
            ...
            tabelaHTML += `<tr>
                ...
                <td>${campanha.impressoes || 0}</td><td>${campanha.cliques || 0}</td><td>${cpcMedio}</td><td>${custo}</td>
                <td>${campanha.conversoes || 0}</td><td>${valorConversoes}</td><td style="color: ${corResultado}; font-weight: 500;">${resultado}</td><td style="color: ${corResultado}; font-weight: 500;">${roi}</td>
            </tr>`;
        });
    }
